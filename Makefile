...
.PHONY: generate prune build test lint checkstyle format analyze upgrade docs clean
...
lint:
	mvn checkstyle:check spotbugs:check -q -B

checkstyle:
	mvn checkstyle:check -q -B
...
docs:
	mvn javadoc:javadoc -q -B -DfailOnWarnings=true -Ddoclint=all -DreportOutputDirectory=.out -DdestDir=docs
