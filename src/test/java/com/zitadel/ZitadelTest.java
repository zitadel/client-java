 ...
 * properly registered as fields in the {@link Zitadel} class.
 *
 * <p>This test dynamically scans the "com.zitadel.api" package using ClassGraph (excluding any
 * classes whose names end with "Test" and only including those whose names end with "ServiceApi").
 * It then uses reflection on a {@link Zitadel} instance to collect the types of all fields that
 * belong to the "com.zitadel.api" package. The test asserts that the set of expected API service
 * classes exactly matches the set of actual service classes registered in {@code Zitadel}.
 *
 * <p>This ensures that all API services are correctly registered in the {@code Zitadel} class
 * without duplicates, which is critical for the correct functioning of the SDK.
 */
class ZitadelTest {
