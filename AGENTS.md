...
- Generator: `java-plus`, run from the `openapi-generator-plus:enhanced` image.
...
Hand-written here: the bespoke authenticators under `src/main/java/com/zitadel/auth/`, their tests, the
...
devbox run -- make test       # mvn test -q -B
...
the generator supports. The types live in `com.zitadel.errors`:
...
the trace-context tests). This repo keeps its own `pom.xml`, so the generator cannot add
